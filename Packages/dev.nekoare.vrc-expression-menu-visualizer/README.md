...
1. Unity Editorで「Tools」→「メニュー整理ツール」→「安定版」を選択
...
最新版-betaには以下の高度な機能が含まれます:

- **マーカーベース管理**: 3種類のマーカーコンポーネントで項目の状態を永続化
- **除外/非除外ワークフロー**: MA Menu Item/Installerを除外/非除外として管理
- **Unity Hierarchy直接編集**: 編集モードでHierarchy上のGameObjectを直接操作
- **自動MA変換**: 保存時にExpression MenuをMA構造へ自動変換

詳細な使用方法は、最新版-betaウィンドウ内のヘルプを参照してください。
